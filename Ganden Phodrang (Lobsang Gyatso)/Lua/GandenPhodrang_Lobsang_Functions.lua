...
function OnPlayerDoTurn_SpyActivities(iPlayer)
	...
	-- Sync Amchi garrison state
	SyncAmchiGarrisonState(player)
end

-- Helper function to sync Amchi garrison state with actual city garrisons
function SyncAmchiGarrisonState(player)
	...
	-- Track which garrisons are currently valid
	local validGarrisons = {}

	-- Check all cities for actual Amchi garrisons
	for city in player:Cities() do
		local cityID = city:GetID()
		local garrisonedUnit = city:GetGarrisonedUnit()

		if garrisonedUnit and garrisonedUnit:GetUnitType() == unitAmchiID then
			local unitID = garrisonedUnit:GetID()
			validGarrisons[cityID] = unitID

			-- Update tracking and bonuses if not already tracked
			if tAmchiGarrison[iPlayer][cityID] ~= unitID then
				tAmchiGarrison[iPlayer][cityID] = unitID

				-- Calculate and apply defense bonus
				local religion = GetPlayerReligion(player)
				local followers = CountFollowersInCity(city, religion)
				local defenseBonus = followers * DEFENSE_PER_FOLLOWER

				city:SetNumRealBuilding(BUILDING_AMCHI_DEFENSE, math.min(defenseBonus / 10, 20))
				city:SetNumRealBuilding(BUILDING_AMCHI_MARCH, 1)
			end
		end
	end

	-- Remove outdated garrison tracking and bonuses
	for cityID, unitID in pairs(tAmchiGarrison[iPlayer]) do
		if not validGarrisons[cityID] or validGarrisons[cityID] ~= unitID then
...
function OnUnitSetXY_Amchi(iPlayer, iUnit, iX, iY)
	...
	if plot and plot:IsCity() then
		...
		if city and city:GetOwner() == iPlayer and city:GetGarrisonedUnit() == unit then
			-- Unit is garrisoned
			local cityID = city:GetID()
...
function OnUnitKilled_AmchiHeal(iPlayer, iUnit, iUnitType, iX, iY, bDelay, iKiller)
	...
	if iKiller ~= -1 and iPlayer ~= iKiller then
		return
	end -- Allow expended units (iKiller == -1) or killed by own player
...
function UpdateDansaBonusAlways(player, city)
	...
	if not city:IsHasBuilding(BUILDING_DANSA) then
